...
export async function POST(req: NextRequest) {
  ...
  const BATCHABLE_CHAINS = new Set(["avax", "bnb", "eth", "mantle", "injective", "monad", "scroll"]);
  if (!BATCHABLE_CHAINS.has(chain)) {
    return NextResponse.json(
      {
        error: `chain "${chain}" is not batchable. Batch-supported chains: avax, bnb, eth, mantle, injective, monad, scroll. For xlayer / stable use /api/relay in a client-side loop.`,
