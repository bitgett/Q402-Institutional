...
describe.skipIf(!available)("MCP two-phase consent Zod<->JSON schema parity", () => {
  ...
  it("finds the nine fund-moving consent tools plus clear-delegation", () => {
    // Nine FUND-MOVING tools use two-phase consent - pay, batch_pay,
    // bridge_send (CCIP), oft_send (LayerZero OFT), request_pay, yield_deposit,
    // yield_withdraw, and stake + unstake (both live in stake.ts). clear-delegation
    // is NOT fund-moving but adopted the SAME token-bound consent because an
    // undelegate broadcasts a real tx and, on Ethereum, bills the Gas Tank, so it
    // belongs to the two-phase SET without changing the fund-moving count.
