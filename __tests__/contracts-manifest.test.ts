...
describe("contracts.manifest.json ↔ public SDK", () => {
  ...
  it("SDK uses the unified TransferAuthorization witness type for all 12 chains", () => {
  ...
  // ── Injective USDC + USDT policy (locks the SDK ↔ relay route ↔ manifest invariant) ──
  // Native Circle USDC (CCTP) shipped on Injective EVM 2026-06; both tokens are
  // now supported. If any of the three surfaces drift apart, this catches it.
  describe("Injective USDC + USDT policy", () => {
    it("manifest declares supportedApiTokens === USDC + USDT", () => {
      const m = manifest.chains.injective as { supportedApiTokens?: string[] };
      expect([...(m.supportedApiTokens ?? [])].sort()).toEqual(["USDC", "USDT"]);
    });

    it("SDK injective supportedTokens === USDC + USDT", () => {
      ...
      const tokens = m![1].split(",").map(s => s.trim().replace(/['"]/g, "")).sort();
      expect(tokens).toEqual(["USDC", "USDT"]);
    });

    it("relay route allowlist includes injective USDC + USDT", () => {
      const route = readFileSync(
        resolve(__dirname, "..", "app", "api", "relay", "route.ts"),
        "utf8",
      );
      expect(route).toMatch(/CHAIN_TOKEN_ALLOWLIST/);
      const m = route.match(/injective:\s*\[([^\]]+)\]/);
      expect(m, "relay route should declare an injective allowlist").not.toBeNull();
      const toks = m![1].split(",").map(s => s.trim().replace(/['"]/g, "")).sort();
      expect(toks).toEqual(["USDC", "USDT"]);
    });
  });
});
