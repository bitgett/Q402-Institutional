...
describe("/api/relay/batch route", () => {
  ...
  it("rejects non-batchable chains (xlayer / stable) with CHAIN_NOT_BATCHABLE", () => {
    ...
    // SDK + Node client + MCP tool schema all agree on the same 10-chain
    // set (avax / bnb / eth / mantle / injective / monad / scroll / arbitrum / base / robinhood);
    ...
    expect(batchRouteSrc).toMatch(/BATCHABLE_CHAINS[\s\S]*?"avax"[\s\S]*?"bnb"[\s\S]*?"eth"[\s\S]*?"mantle"[\s\S]*?"injective"[\s\S]*?"monad"[\s\S]*?"scroll"[\s\S]*?"arbitrum"[\s\S]*?"base"[\s\S]*?"robinhood"/);
...
describe.skipIf(!mcpAvailable)("MCP server q402_batch_pay registration", () => {
  ...
  it("input schema restricts chain to the 10-chain batchable set", () => {
    ...
    // robinhood is batchable (standard eip7702 chain, USDG-only).
    expect(mcpBatchToolSrc).toMatch(/z\.enum\(\[\s*"avax",\s*"bnb",\s*"eth",\s*"mantle",\s*"injective",\s*"monad",\s*"scroll",\s*"arbitrum",\s*"base",\s*"robinhood"\s*\]\)/);
    expect(mcpBatchToolSrc).toMatch(/enum:\s*\[\s*"avax",\s*"bnb",\s*"eth",\s*"mantle",\s*"injective",\s*"monad",\s*"scroll",\s*"arbitrum",\s*"base",\s*"robinhood"\s*\]/);
