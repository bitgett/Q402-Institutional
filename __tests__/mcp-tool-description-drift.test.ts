...
describe.skipIf(!available)("MCP tool descriptions match actual server policy", () => {
  describe("PAY_TOOL.description", () => {
    ...
    it("documents the multichain → full 11-chain matrix", () => {
      ...
      expect(paySrc).toMatch(/avax.+bnb.+eth.+xlayer.+stable.+mantle.+injective.+monad.+scroll.+arbitrum/i);
    });
  ...
  describe("QUOTE_TOOL.description", () => {
    ...
    it("documents the 12-chain quote surface", () => {
      expect(quoteSrc).toMatch(/12 chains/);
