 ...
 *
 * The footgun (spec MED-2): if LISTA_YIELD_ENABLED is flipped while YIELD_IMPL_BNB
 * still points at the Aave impl (which has no supplyToErc4626), every BNB Lista
 * deposit signs an ERC-4626 witness, delegates to the Aave impl, and reverts AFTER
 * the relayer paid gas. This script asserts the currently-wired YIELD_IMPL_BNB IS
 * the Lista ERC-4626 impl with the right domain + allowlist, via eth_call (free).
 *
 * Reads YIELD_IMPL_BNB from .env.local. Read-only. Exit 0 = safe to flip.
 */
...
const IMPL = (env.match(/^YIELD_IMPL_BNB=(0x[0-9a-fA-F]{40})/m)?.[1] || "").trim();
...
if (!IMPL) {
  console.error("YIELD_IMPL_BNB is not set in .env.local — deploy the Lista ERC-4626 impl first (scripts/deploy-yield-impl.mjs --chain bnb), then set it. DO NOT flip LISTA_YIELD_ENABLED yet.");
  process.exit(1);
}
...
try {
  ...
  console.error("could not read the impl — is YIELD_IMPL_BNB a deployed contract on BSC?", e?.shortMessage || e?.message);
  process.exit(1);
}
...
if (!ok) {
  console.error("\n! WIRING MISMATCH — YIELD_IMPL_BNB is NOT the Lista ERC-4626 impl (or its allowlist is wrong). DO NOT set LISTA_YIELD_ENABLED=true; deposits would revert after paying gas.");
  process.exit(1);
}
console.error("\nOK — YIELD_IMPL_BNB is the Lista ERC-4626 impl with the Gauntlet USDT + Lista USDC allowlist. Safe to set LISTA_YIELD_ENABLED=true.");
